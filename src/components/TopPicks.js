  ...
  return (
    <div>
      ...
      <div className=" lg:flex max-w-[1520px] m-auto py-2 px-2">
        <Splide
          options={{ perPage: isMobile ? 1 : 4, gap: "0.5rem", drag: "free", arrows: isMobile ? true : false }}
        >
          {topPicks.map((item) => {
            return (
              <SplideSlide key={item.id}>
                <div className="rounded-3xl relative">
                  ...
                  <img
                    className="h-[200px] w-full object-cover rounded-lg cursor-pointer hover:scale-105 ease-out duration-300"
                    src={item.img}
                    alt={item.title}
                  />
                </div>
